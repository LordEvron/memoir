	...
	public static String storeThumbnail(Context c, String path) {
		...
		if(useExternal) {
			...
			if (Environment.MEDIA_MOUNTED.equals(state)) {
			    ...
			    File mediaStorageDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES), "Memoir/.thumbnails");
			    if (! mediaStorageDir.exists()){
			        if (! mediaStorageDir.mkdirs()){
			            Log.d("Memoir", "failed to create directory");
			            return null;
			...
			Log.d("asd", "getDataDirectory > " + c.getFilesDir().getAbsolutePath());
			File mediaStorageDir = new File(c.getFilesDir().getAbsolutePath(), "Memoir/.thumbnails");
		    if (! mediaStorageDir.exists()){
		        if (! mediaStorageDir.mkdirs()){
		            Log.d("Memoir", "failed to create directory");
		            return null;
